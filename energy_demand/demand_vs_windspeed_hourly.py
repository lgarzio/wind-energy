...
wind power at 160m at the "Endurance" point in the middle of the WEA off of Atlantic City, NJ. Separated by hour.
"""
...
def main(sDir, pjm_dir, sdate, edate):
    ...
    # create the colormap
    cmap = plt.get_cmap('rainbow', len(hours))  # nipy_spectral rainbow

    for pv in plt_vars:
        savedir = os.path.join(sDir, f'demand_vs_{pv}', 'hourly')
        ...
        for hour in hours:
            ...
            ax.scatter(merged[pv], merged.mw, color='lightgray', s=25)
            ...
            if pv == 'power':
                ...
                xposition = [3, 10.9]
                for xc in xposition:
                    ax.axvline(x=xc, color='dimgray', linestyle='--')
            ax.set_xlabel(xaxis)

            save_filename = 'demand_vs_{}_hourly_{}-{}-hour{:02d}.png'.format(pv, sdate.strftime('%Y%m%d'),
                                                                              edate.strftime('%Y%m%d'), hour)  # -AE
